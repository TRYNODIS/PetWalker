package hu.nagyi.petwalker.map
...
import hu.nagyi.petwalker.MainMarkerClusterItem
import hu.nagyi.petwalker.R
import hu.nagyi.petwalker.data.LocationData
import hu.nagyi.petwalker.databinding.ActivityMapsBinding
...
class MapsActivity : AppCompatActivity(), OnMapReadyCallback,
    ...
    private lateinit var clusterManager: ClusterManager<MainMarkerClusterItem>
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        this.binding.geocodeBtn.setOnClickListener {
            if (LocationData.previousLocation != null) {
                this.geocodeLocation(LocationData.previousLocation!!)
            }
        }
...
/*        val markerHungary = LatLng(47.0, 19.0)
    ...
    private fun addMarkerClusterItems() {
        ...
        for (i in 0..10) {
            ...
            lng += offset
            val offsetItem =
                MainMarkerClusterItem(lat, lng, "Title $i", "Snippet $i")
    ...
    override fun onNewLocation(location: Location) {
        ...
        if (LocationData.previousLocation != null && this.startAltitude == null) {
            this.startAltitude = LocationData.previousLocation!!.altitude
        }
        if (this.startAltitude != null) {
            this.actualAltitude = LocationData.previousLocation!!.altitude
        }
        if (this.actualAltitude != null && this.startAltitude != null) {
            this.difAltitude = this.actualAltitude!! - this.startAltitude!!
        }

        if (LocationData.previousLocation != null && location.accuracy < 20) {
            if (LocationData.previousLocation!!.time < location.time) {
                this.distance += LocationData.previousLocation!!.distanceTo(location)
